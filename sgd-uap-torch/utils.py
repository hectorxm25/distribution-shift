...
def model_cifar(model_name, ckpt_path):
    ...
    # Attempt to load state_dict based on common Robustness library patterns
    state_dict_to_load = None
    if 'state_dict' in checkpoint:
        state_dict_to_load = checkpoint['state_dict']
    elif 'model' in checkpoint and isinstance(checkpoint['model'], dict):
        # If checkpoint['model'] itself contains 'state_dict', use that
        if 'state_dict' in checkpoint['model']:
            state_dict_to_load = checkpoint['model']['state_dict']
        else:
            # Otherwise, assume checkpoint['model'] *is* the state_dict
            state_dict_to_load = checkpoint['model']
    else:
        # Fallback: assume checkpoint is the state_dict itself
    ...
    if state_dict_to_load is not None:
        for k, v in state_dict_to_load.items():
            name = k
            # Strip common prefixes from Robustness library
            if name.startswith('attacker.model.'):
                name = name[len('attacker.model.'):]
            if name.startswith('normalizer.model.'): # Note: using 'if' not 'elif' in case of nested prefixes
                name = name[len('normalizer.model.'):]
            
            # After attacker/normalizer stripping, 'model.' might be the outermost useful prefix
            if name.startswith('model.'):
                name = name[len('model.'):]
            
            # The target model is DataParallel(ResNet18()), so keys must start with 'module.'
            # If 'module.' is already there (e.g. 'module.conv1.weight'), keep it.
            # If not (e.g. 'conv1.weight'), add it.
            if not name.startswith('module.'):
                name = f'module.{name}'
            
            new_state_dict[name] = v
        
    model.load_state_dict(new_state_dict, strict=False) # Use strict=False to ignore non-model keys
    
    # best_acc and start_epoch might not be in the Robustness checkpoint or under these exact keys.
