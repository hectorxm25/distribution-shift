...
def create_masks_loader(config, model_path, loader):
    ...
    for batch in loader:
        images, labels = batch
        _, _, mask_tensors, _, mask_labels, adv_predicted_labels = create_masks_batch(
            config, model_path, images, labels
        )
...
def check_mask_accuracy(loader, model_path):
    ...
    for images, labels in loader:
        ...
        for i in range(len(images)):
            ...
            gradient_mask, mask, _, _, _ = create_mask(
                ATTACK_PARAMS, model_path, [(image, label)], OUTPUT_DIR
            )
...
def check_mask_accuracy_both_batch(loader, model_path):
    ...
    for images, labels in loader:
        ...
        _, _, masks, _, orig_labels, adv_labels = create_masks_batch(
            ATTACK_PARAMS, model_path, images, labels
        )
...
if __name__ == "__main__":
    ...
    natural_accuracy, adversarial_accuracy = check_mask_accuracy_both_batch(
        training_loader,
