...
def test_mask_accuracy(model_path, loader, attack_config=ATTACK_PARAMS):
    ...
    for images, labels in loader:
        _, _, masks, _, _, adv_labels = mask_gen.create_masks_batch(
            attack_config, model_path, images, labels
        )
    ...
    print(
        f"Natural accuracy: {accuracy_natural}, Adversarial accuracy: {accuracy_adversarial}"
    )
...
def test(
    model_path, loader, dset_path="/home/gridsan/hmartinez/distribution-shift/datasets"
):
    # set up dataset again
    dataset = datasets.CIFAR(data_path=dset_path)
    # load trained model
    model, _ = model_utils.make_and_restore_model(
        arch="resnet18", dataset=dataset, resume_path=model_path
    )
...
def compare_losses(natural_model_path, adversarial_model_path):
    ...
    natural_model, _ = model_utils.make_and_restore_model(
        arch="resnet18", dataset=dataset, resume_path=natural_model_path
    )
    adversarial_model, _ = model_utils.make_and_restore_model(
        arch="resnet18", dataset=dataset, resume_path=adversarial_model_path
    )
...
def plot_loss_histogram(natural_losses, adversarial_losses, save_path):
    ...
    plt.title(
        "Distribution of Logit-Transformed Losses: Natural vs Adversarial Model",
        fontsize=14,
        pad=20,
    )
    ...
    plt.axvline(
        nat_mean, color="blue", linestyle="dashed", alpha=0.8, label="Natural Mean"
    )
    plt.axvline(
        adv_mean, color="red", linestyle="dashed", alpha=0.8, label="Adversarial Mean"
    )
...
def plot_single_loss_histogram(losses, save_path, title):
    ...
    mean = np.mean(losses)
    plt.axvline(
        mean, color="red", linestyle="dashed", alpha=0.8, label=f"Mean: {mean:.3f}"
    )
    plt.legend(fontsize=10)
...
def get_natural_and_mask_confidence(
    model_path, images, labels, masks, adv_labels, apply_softmax=False
):
    """
    ...
    with torch.no_grad():
        ...
        for i in range(len(outputs)):
            natural_confidence.append(
                outputs[i][labels[i]]
            )  # now natural confidence is a list of confidence values for the natural label with the natural images
    ...
    with torch.no_grad():
        ...
        outputs = outputs.cpu()
        for i in range(len(outputs)):
            mask_adv_confidence.append(
                outputs[i][adv_labels[i]]
            )  # now mask adversarial confidence is a list of confidence values for adversarial label with the mask images
    ...
    with torch.no_grad():
        ...
        outputs = outputs.cpu()
        for i in range(len(outputs)):
            natural_adv_confidence.append(
                outputs[i][adv_labels[i]]
            )  # now natural adversarial confidence is a list of confidence values for adversarial label with the natural images
...
def plot_loss_histogram_test_vs_train_multiple_batches(
    train_losses, test_losses, save_path, title
):
    plt.figure(figsize=(10, 6))
...
def plot_loss_histogram_natural_vs_mask(natural_losses, mask_losses, save_path, title):
    ...
    # Plot histograms
    plt.hist(
        natural_phi, bins=50, alpha=0.5, density=True, label="Natural", color="blue"
    )
...
if __name__ == "__main__":
    ...
    # test mask accuracy
    MODEL_PATH = (
        "/home/gridsan/hmartinez/distribution-shift/models/natural/149_checkpoint.pt"
    )
    _, training_loader, test_loader = load_dataset(
